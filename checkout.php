...
if(isset($_POST['product_ids']) && isset($_POST['product_names']) && isset($_POST['product_prices'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Display the product details and image
    ...
    // Check if the cart session variable exists and is not empty
        elseif (isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
            // Convert selected products string into an array
            $selected_products = isset($_POST['selected_products']) ? explode(',', $_POST['selected_products']) : [];

            // Loop through each cart item
            foreach ($_SESSION['cart'] as $cart_item) {
                // Check if the product ID is included in the selected products array
                if (in_array($cart_item['product_id'], $selected_products)) {
                    $cart_data .= '<div class="container p-3 mb-3">';
                    $cart_data .= '<div class="row">';
                    $cart_data .= '<div class="col-md-3">';
                    $cart_data .= '</div>';
                    $cart_data .= '<div class="col-md-2">';
                    $cart_data .= '<img src="data:image/jpeg;base64,' . $cart_item['image_data'] . '" class="img-fluid" alt="Product Image">';
                    $cart_data .= '</div>';
                    $cart_data .= '<div class="col-md-4">';
                    $cart_data .= '<div class="card">';
                    $cart_data .= '<div class="card-body">';
                    $cart_data .= "<p class='card-text'><strong>Product Name:</strong> " . $cart_item['product_name'] . "</p>";
                    $cart_data .= "<p class='card-text'><strong>Price:</strong> " . $cart_item['product_price'] . "</p>";
                    if (isset($cart_item['product_color'])) {
                        $cart_data .= "<p class='card-text'><strong>Color:</strong> " . $cart_item['product_color'] . "</p>";
                    }
                    if (isset($cart_item['product_size'])) {
                        $cart_data .= "<p class='card-text'><strong>Size:</strong> " . $cart_item['product_size'] . "</p>";
                    }
                    $cart_data .= '</div>';
                    $cart_data .= '</div>';
                    $cart_data .= '</div>';
            ...
            // If the cart is empty, set a message
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form id="checkoutForm" method="post" action="completepurchase.php">
                    ...
                    <div class="form-group">
                        <label for="address">Complete Address (PUROK#, BRGY, MUNICIPALITY, PROVINCE, ZIP, LANDMARK):</label>
                        <textarea class="form-control" id="address" name="landmark" rows="3" required></textarea>
                    </div>
                    <!-- Hidden input field to hold the order ID -->
                    <input type="hidden" id="orderId" name="orderId" value="123"> <!-- Replace '123' with your actual order ID -->
                ...
                <h5>Message<a href="https://www.facebook.com/profile.php?id=61555989808165&mibextid=ZbWKwL" class="my-2"> Thrift Things </a>for Payment</h5>
            </div>
        </div>
    </div>
</div>
 <!-- STORING PRODUCT INFO IN TABLE -->
 <?php
 include 'conn.php';
 function store_cart_data($conn) {
    // Check if the cart session variable exists and is not empty
    if (isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
        // Loop through each cart item
        foreach ($_SESSION['cart'] as $cart_item) {
            // Extract details from the cart item
            ...
            // Prepare and bind
            $stmt = $conn->prepare("INSERT INTO order_summaries (product_name, product_price, product_color, product_size, image_data) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sdsss", $product_name, $product_price, $product_color, $product_size, $image_data);
        ...
        // If the cart is empty, you might want to handle this case
        // For example, log a message or set a session variable
        error_log("Cart is empty, nothing to store.");
    }
}
